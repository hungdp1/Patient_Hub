...
const svc = __importStar(require("./prescriptions.service"));
function actor(req) {
    if (!req.user)
...
async function createPrescription(req, res) {
    const data = await svc.createPrescription(req.body, actor(req));
...
async function getPrescription(req, res) {
    const data = await svc.getPrescription(req.params['id'], actor(req));
    res.json({ data });
}
async function updatePrescription(req, res) {
    const data = await svc.updatePrescription(req.params['id'], req.body, actor(req));
    res.json({ data });
}
