...
const tickFail = (m) => console.log(`  ❌ ${m}`);
const tickPass = (m) => console.log(`  ✅ ${m}`);
const tickWarn = (m) => console.log(`  ⚠️  ${m}`);
async function main() {
    console.log('🔍 PayOS diagnostic');
    header('1. Biến môi trường');
    row('NODE_ENV', env_1.env.NODE_ENV);
    row('PAYOS_CLIENT_ID', env_1.env.PAYOS_CLIENT_ID, env_1.env.PAYOS_CLIENT_ID ? '' : '← BẮT BUỘC');
    row('PAYOS_API_KEY', env_1.env.PAYOS_API_KEY ? `(${env_1.env.PAYOS_API_KEY.length} ký tự)` : '', env_1.env.PAYOS_API_KEY ? '' : '← BẮT BUỘC');
    row('PAYOS_CHECKSUM_KEY', env_1.env.PAYOS_CHECKSUM_KEY ? `(${env_1.env.PAYOS_CHECKSUM_KEY.length} ký tự)` : '', env_1.env.PAYOS_CHECKSUM_KEY ? '' : '← BẮT BUỘC');
    row('PAYOS_RETURN_URL', env_1.env.PAYOS_RETURN_URL);
    row('PAYOS_CANCEL_URL', env_1.env.PAYOS_CANCEL_URL);
    header('2. Kiểm tra điều kiện');
    let ok = true;
    if (!env_1.env.PAYOS_CLIENT_ID) {
    ...
    if (!env_1.env.PAYOS_CHECKSUM_KEY) {
    ...
    if (env_1.env.PAYOS_RETURN_URL.includes('localhost') && env_1.env.NODE_ENV === 'production') {
    ...
    try {
        ...
        tickFail(`Tạo link lỗi: ${e.message}`);
  ...
  → Đăng ký webhook URL với PayOS (1 lần, sau khi deploy):
      curl -X POST https://api-merchant.payos.vn/confirm-webhook \\
        -H "x-client-id: ${env_1.env.PAYOS_CLIENT_ID}" \\
        -H "x-api-key: ${env_1.env.PAYOS_API_KEY}" \\
...
//# sourceMappingURL=payos-diag.js.map