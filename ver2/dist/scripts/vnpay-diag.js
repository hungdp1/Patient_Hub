...
async function main() {
    ...
    row('NODE_ENV', env_1.env.NODE_ENV);
    row('VNP_HOST', env_1.env.VNP_HOST);
    row('VNP_TMN_CODE', env_1.env.VNP_TMN_CODE, env_1.env.VNP_TMN_CODE ? '' : '← BẮT BUỘC');
    row('VNP_HASH_SECRET', env_1.env.VNP_HASH_SECRET ? `(${env_1.env.VNP_HASH_SECRET.length} ký tự)` : '', env_1.env.VNP_HASH_SECRET ? '' : '← BẮT BUỘC');
    row('VNP_RETURN_URL', env_1.env.VNP_RETURN_URL);
    row('VNP_ALLOWED_IPS', env_1.env.VNP_ALLOWED_IPS, env_1.env.VNP_ALLOWED_IPS ? '' : '(whitelist tắt)');
    header('2. Kiểm tra điều kiện');
    let ok = true;
    if (!env_1.env.VNP_TMN_CODE) {
        fail('VNP_TMN_CODE trống — đăng ký terminal sandbox tại https://sandbox.vnpayment.vn');
        ok = false;
    }
    else if (env_1.env.VNP_TMN_CODE.length < 4 || env_1.env.VNP_TMN_CODE.length > 20) {
        warn(`VNP_TMN_CODE độ dài ${env_1.env.VNP_TMN_CODE.length} — VNPay thường cấp 8 ký tự`);
    }
    else {
        pass(`VNP_TMN_CODE hợp lệ (${env_1.env.VNP_TMN_CODE.length} ký tự)`);
    }
    if (!env_1.env.VNP_HASH_SECRET) {
        fail('VNP_HASH_SECRET trống — lấy từ portal cùng terminal');
        ok = false;
    }
    else if (env_1.env.VNP_HASH_SECRET.length < 16) {
        warn(`VNP_HASH_SECRET quá ngắn (${env_1.env.VNP_HASH_SECRET.length}) — VNPay sandbox thường 32 ký tự HMAC, production cấp 64`);
    }
    else {
        pass(`VNP_HASH_SECRET hợp lệ (${env_1.env.VNP_HASH_SECRET.length} ký tự)`);
    }
    if (!env_1.env.VNP_HOST.startsWith('https://')) {
        warn(`VNP_HOST không bắt đầu bằng https:// — kiểm tra lại`);
    }
    else if (env_1.env.VNP_HOST.includes('sandbox') &&
    ...
    if (env_1.env.VNP_RETURN_URL.includes('localhost') && env_1.env.NODE_ENV === 'production') {
        fail('VNP_RETURN_URL còn là localhost ở production');
        ok = false;
    }
    else {
        pass(`VNP_RETURN_URL = ${env_1.env.VNP_RETURN_URL}`);
    }
    if (!ok) {
    ...
    try {
        const dc = (0, vnpay_1.vnpayClient)().defaultConfig;
        ...
        fail(`Không khởi tạo được VNPay client: ${e.message}`);
        process.exit(1);
    }
    header('4. URL thanh toán mẫu (50.000đ)');
    const txnRef = (0, vnpay_1.generateTxnRef)();
    const url = (0, vnpay_1.buildPaymentUrl)({
        amount: 50000,
        txnRef,
...
//# sourceMappingURL=vnpay-diag.js.map