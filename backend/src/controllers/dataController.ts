...
import { getRealTimeIO } from '../realtime/socketServer';
...
export const createAppointment = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (appointment.doctorId) {
      io.to(`user:${appointment.doctorId}`).emit('appointment:assigned', { appointment });
    }
    io.to('role:ADMIN').emit('appointment:created_admin', { appointment });
  } catch (error) {
    console.warn('Realtime emit failed for appointment create:', error);
  }
...
export const updateAppointment = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (appointment.patientId) {
      io.to(`user:${appointment.patientId}`).emit('appointment:status_changed', { appointment });
    }
    if (appointment.doctorId) {
      io.to(`user:${appointment.doctorId}`).emit('appointment:status_changed', { appointment });
    }
  } catch (error) {
    console.warn('Realtime emit failed for appointment update:', error);
  }
...
export const createLabResult = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    const io = getRealTimeIO();
    if (labResult.patientId) {
      io.to(`user:${labResult.patientId}`).emit('lab:result_available', { labResult });
    }
    if (labResult.doctorId) {
      io.to(`user:${labResult.doctorId}`).emit('lab:result_available', { labResult });
    }
  } catch (error) {
    console.warn('Realtime emit failed for lab result create:', error);
  }
...
export const updateLabResult = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    console.warn('Realtime emit failed for lab result update:', error);
  }
...
export const createMedicalRecord = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    const io = getRealTimeIO();
    if (record.patientId) {
      io.to(`user:${record.patientId}`).emit('medical:record:updated', { record });
    }
    if (record.doctorId) {
      io.to(`user:${record.doctorId}`).emit('medical:record:updated', { record });
    }
  } catch (error) {
    console.warn('Realtime emit failed for medical record create:', error);
  }
...
export const updateMedicalRecord = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    console.warn('Realtime emit failed for medical record update:', error);
  }
